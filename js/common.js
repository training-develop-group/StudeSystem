	...
	getMenu: function(options) {
		if (options && options != null) {
			...
			if (localStorage.getItem('userName') == undefined) {
				navHtml.push('<span class="username">请登录</span>');
			} else {
				navHtml.push('<span class="username">' + localStorage.getItem('userName') + '</span>');
			}

			navHtml.push('<img src="./../imgs/et.png" class="logout">');
			navHtml.push('<div class="changPassWord hidden">');
			navHtml.push('<span class="Triangle"></span>');
			navHtml.push('<span class="changePasWod">修改密码</span>');
			navHtml.push('</div>');
			navHtml.push('</div>');
		...
		$('.userimg').hover(function() {
			$('.changPassWord').slideDown("slow");
			$('.changePasWod').off('click').on('click', function() {
				window.location.href = "../ChangePassWord/ChangePassWord.html";
			})
		});
		$('.changPassWord').hover(function() {

		}, function() {
			$('.changPassWord').slideUp("slow");
		});
	...
	logOut: function() {
		$('body').delegate('.logout', 'click', function() {
			layer.open({
				...
				area: ['450px', '180px'],
				move: false,
				title: ['退出登陆', 'background-color: #289ef0;text-align: center;font-size: 20px;color:white;'],
				closeBtn: 0,
				content: "<p class=''>是否确认退出</p><div class='btn-box'><button class='layui-btn layui-btn-sm layui-btn-normal ok'>确认</button><button class='layui-btn layui-btn-sm no'>取消</button></div>",
	...
	layuiOpen: function(options) {
		if (options && options != null) {
			layer.open({
				...
				closeBtn: 0,
				skin: 'delete',
				title: ['删除', 'background-color: #289ef0;text-align: center;font-size: 20px;color:white;'],
		...
		if (options && options != null) {
			...
			layer.open({
				...
				content: '<div class="common-inputLocation">' +
					...
					'<input type="text" maxlength="50" autocomplete="off" class="layui-input common-acquiredValue">' +
					'</div>' +
